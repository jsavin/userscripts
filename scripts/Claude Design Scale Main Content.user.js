...
(function () {
  ...
  //
  // Zooming the iframe element directly scales only the rendered preview content.
  // The clip wrapper (.sc-hsxRXz) gets overflow: visible so the larger iframe
  // isn't cropped, and its height expands via a MutationObserver that sets a
  // CSS custom property from the iframe's natural height.
  //
  // The screen tab bar ("Sign in / Shop / Quickview") lives inside the iframe
  ...
  // Read each iframe's natural height and expose it as a CSS var on its wrapper
  // so the calc() above can expand the wrapper height to match the zoom.
